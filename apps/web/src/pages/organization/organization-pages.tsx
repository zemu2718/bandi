...
export function OrganizationPage() {
  ...
  return <>
    <PageHeader title="Team" description="管理 Team、所属 Agent 和共享资产。" action={state.teams.length ? <Button onClick={createTeam}>创建 Team</Button> : undefined} />
    ...
    })}</section> : <div className="grid min-h-80 place-items-center"><EmptyState className="w-full max-w-lg border-0" title="还没有 Team" description="创建 Team 后，可以将 Agent 加入其中并管理共享资产。" action={<Button onClick={createTeam}>创建 Team</Button>} /></div>}
  </>
}
...
export function TeamDetailPage() {
  ...
  return <>
    <PageHeader backTo="/organization" title={team.name} description="此处列出属于该 Team 的 Agent。Agent 只有选择共享资产后才会使用它。" action={<Button variant="outline" onClick={() => dispatch({ type: 'OPEN_DIALOG', dialog: { kind: 'organization', entity: 'team', id: team.id, mode: 'edit' } })}>编辑 Team</Button>} />
    <div className="grid gap-5 lg:grid-cols-2">
      <section className="panel p-5"><h3 className="font-semibold">Team 信息</h3><FieldRow label="来源">{team.id === 'team-personal' ? 'Bandi 固定 Team，始终存在' : state.runtime === 'desktop' ? 'Bandi 本机受管配置' : '当前页面演示'}</FieldRow><FieldRow label="使命">{team.mission || '未设置'}</FieldRow><FieldRow label="边界">{team.boundary || '未设置'}</FieldRow><p className="mt-4 text-xs leading-5 text-muted-foreground">成员关系由 Agent 的 Team 归属决定。共享资产必须由 Agent 显式引用，Team 归属不会自动授予权限。</p></section>
