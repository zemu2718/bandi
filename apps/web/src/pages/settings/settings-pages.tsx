...
const settingsSections = [['tools', 'AI 工具'], ['terminal', '终端偏好'], ['recovery', '数据与恢复'], ['appearance', '外观']] as const
...
export function SettingsPage() {
  ...
  return <div className={usesTabbedWorkspace ? 'lg:flex lg:h-[calc(100dvh-6.5rem)] lg:min-h-0 lg:flex-col lg:overflow-hidden min-[1281px]:h-[calc(100dvh-7rem)]' : undefined}>
    <PageHeader title="设置" description="管理 AI 工具、终端偏好、本机数据恢复与外观。" />
...
const desktopDataTabs = [
  ...
  { id: 'reset', label: '重置 Bandi' },
] as const

type DataTabId = typeof webDataTabs[number]['id'] | typeof desktopDataTabs[number]['id']
...
function ConfigurationAndBackupSection() {
  ...
  const defaultTab: DataTabId = desktop ? 'storage' : 'profiles'
  const validTab = (tab: string | null): tab is DataTabId => tabs.some((item) => item.id === tab)
  const initialTab = validTab(requestedTab) ? requestedTab : defaultTab
  const contentRef = useRef<HTMLDivElement>(null)
  const [activeTab, setActiveTab] = useState<DataTabId>(initialTab)
  ...
  useEffect(() => {
    const available = desktop ? desktopDataTabs : webDataTabs
    const next: DataTabId = available.some((tab) => tab.id === requestedTab)
      ? requestedTab as DataTabId
      : desktop ? 'storage' : 'profiles'
    setActiveTab(next)
  }, [desktop, requestedTab])
  const showTab = (tab: DataTabId) => {
    setActiveTab(tab)
    const next = new URLSearchParams(params)
    next.set('section', 'recovery')
    next.set('tab', tab)
    setParams(next)
  ...
  return <div className="flex min-h-0 min-w-0 max-w-full flex-col lg:h-full">
    ...
    <div ref={contentRef} data-testid="configuration-backup-scroll-area" className="min-h-0 min-w-0 flex-1 space-y-5 pb-8 lg:overscroll-contain lg:overflow-y-auto lg:pr-2">
      ...
      {desktop && <EntityTabPanel tabId="reset" activeTab={activeTab} scope="configuration-backup"><FactoryResetPanel /></EntityTabPanel>}
...
function SnapshotRecoveryPanel() {
  if (isDesktopRuntime()) return <><DesktopBackupPanel /><MockBoundaryNote>配置文件快照只保护你明确选择的可写受管配置文件，不包含 Bandi 本机数据、组织与工作区记录、完整 Agent 配置、Agent 长期记忆、工具方案、个性化或凭据，也不能恢复整个 Bandi。</MockBoundaryNote></>
