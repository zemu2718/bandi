...
export function SettingsPage() {
  ...
  return <div className={usesTabbedWorkspace ? 'lg:flex lg:h-[calc(100dvh-6.5rem)] lg:min-h-0 lg:flex-col lg:overflow-hidden min-[1281px]:h-[calc(100dvh-7rem)]' : undefined}>
    ...
    <div className={`grid min-w-0 gap-5 lg:grid-cols-[220px_minmax(0,1fr)] ${usesTabbedWorkspace ? 'lg:min-h-0 lg:flex-1' : ''}`}>
      <nav className="panel h-fit p-2" aria-label="设置分类">{settingsSections.map(([id, label]) => <button type="button" aria-current={section === id ? 'page' : undefined} key={id} onClick={() => setParams(id === 'terminal' ? {} : { section: id })} className={`w-full rounded-md px-3 py-2.5 text-left text-sm ${section === id ? 'bg-foreground font-medium text-background' : 'hover:bg-muted'}`}>{label}</button>)}</nav>
...
function ConfigurationAndBackupSection() {
  ...
  const initialTab = validTab(requestedTab) ? requestedTab : 'storage'
...
function SnapshotRecoveryPanel() {
  if (isDesktopRuntime()) return <><DesktopBackupPanel /><MockBoundaryNote>配置文件快照只保护你明确选择的可写受管配置文件，不包含 Bandi 本机数据、组织与工作区记录、完整 Agent 配置、Agent 长期记忆、个性化或凭据，也不能恢复整个 Bandi。</MockBoundaryNote></>
