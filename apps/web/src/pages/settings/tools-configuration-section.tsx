...
import { HostIntegrationSection } from './host-integration-section'
...
export function ToolsConfigurationSection() {
  ...
  return <>
    ...
    <section className="panel overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-4 border-b border-border p-5"><div><b>AI 编程工具</b><p className="mt-1 text-sm text-muted-foreground">尚未检查工具是否已安装；自定义工具只能保存管理信息，不能准备上下文。</p></div><Button size="sm" variant="outline" onClick={() => setEditor('custom')}><Plus size={15} aria-hidden="true" />添加自定义工具</Button></div><div className="divide-y divide-border">{state.aiClients.map((client) => { const selected = current.clientIds.includes(client.id); return <div key={client.id} className="grid gap-4 p-5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div className="flex items-center gap-3"><AiClientIcon client={client} size={36} tile /><span><b className="block">{client.name}</b><small className="text-muted-foreground">{supportsClientLaunch(client) ? '支持选择 Agent 与可选上下文' : '仅管理配置'}</small></span></div><div className="flex flex-wrap justify-end gap-2">{client.kind === 'custom' && <Button size="sm" variant="outline" disabled={selected || busy} onClick={() => run(() => deleteCustomTool(client.id, revision), '自定义工具已删除')}>删除</Button>}<Button size="sm" variant="outline" disabled={busy} onClick={() => run(() => saveToolPlan({ ...environmentToPlan(current), toolIds: selected ? current.clientIds.filter((id) => id !== client.id) : [...current.clientIds, client.id] }, revision), selected ? '工具已移出方案' : '工具已加入方案')}>{selected ? '移出当前方案' : '加入当前方案'}</Button></div></div> })}</div></section>
