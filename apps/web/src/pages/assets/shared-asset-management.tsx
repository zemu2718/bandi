...
export function SharedAssetDetail({ assetId }: { assetId: string }) {
  ...
  const repair = async () => { setBusy(true); setError(''); try { await repairSharedAssetRegistration({ requestId: crypto.randomUUID(), assetId }); setRegistrationPending(false); navigate('/assets') } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)) } finally { setBusy(false) } }
