...
import { ArrowLeft, Copy, FileDiff, History, KeyRound, Save, Search, ShieldCheck } from 'lucide-react'
...
import { applyAgentConfig, normalizeAgentName, parseAgentComponentRefs, parseAgentContextConfig, parseAgentMcpRefs, parseAgentPermissions, parseAgentRuleRefs, parseAgentSkillRefs, parseAgentSopRefs, serializeAgentConfig, validateAgentName, validateContextPolicy, validateContextWindowTokens, type AgentContextConfig, type AgentIdentityConfig } from '../../agent-config-model'
  ...
  return <>
    <header className="mb-5">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="flex min-w-0 items-center gap-3"><AgentAvatar agent={agent} className="size-12 text-lg" /><div className="min-w-0"><h2 className="text-2xl font-semibold tracking-tight">{agent.name}</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">{identitySummary}</p></div></div>
        <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => dispatch({ type: 'OPEN_DIALOG', dialog: { kind: 'source', agentId: agent.id, section: route.section } })}><Search size={16} aria-hidden="true" />诊断来源</Button><AiClientLaunchAction agentId={agent.id} agentName={agent.name} disabled={agent.status !== 'active'} /></div>
      </div>
    </header>
...
function IdentityTab({ agent }: { agent: FullAgent }) {
  ...
  return <><section className="panel overflow-hidden"><TabHeader title="身份与职责" description="部门、岗位和跨部门服务不形成隐式权限。" editing={editing} onEdit={beginEdit} onCancel={cancel} onSave={save} canSave={!nameError} saveLabel={managedAvatar ? '保存' : '保存到当前页面'} />
...
function ContextTab({ agent }: { agent: FullAgent }) {
  ...
  const description = `${desktopManaged ? '真实保存目标' : '当前页面保存位置'}：${agent.packagePath}config/context.yaml`
...
function MemoryTab({ agent }: { agent: FullAgent }) {
  ...
  return <section className="panel overflow-hidden"><TabHeader title="长期记忆" description={`直接保存到 ${space.path}；每次变更生成不可变 revision。`} editing={true} onEdit={() => {}} onCancel={() => setContent(space.content)} onSave={save} canSave={dirty && !saving} saveLabel={saving ? '保存中…' : isDesktopRuntime() ? '保存' : '保存到当前页面'} /><div className="p-5"><label className="block text-sm font-medium">Memory 正文<textarea className="mt-2 min-h-72 w-full p-4 font-mono text-sm leading-6" value={content} onChange={(event) => setContent(event.target.value)} /></label>{error && <p role="alert" className="mt-3 text-sm text-danger">{error}</p>}<div className="mt-4 flex items-center justify-between gap-3"><StatusBadge tone="success">{space.revision}</StatusBadge>{isDesktopRuntime() && <MemoryRevisionHistory spaceId={space.id} currentRevisionId={space.revision} />}</div></div></section>
}
...
function PermissionsTab({ agent }: { agent: FullAgent }) {
  ...
  const handleResult = (result: SaveConfigResult) => {
    if (result.kind === 'saved' || result.kind === 'unchanged') { commit(result.kind === 'saved' ? '长期权限边界已保存到 Agent 配置' : '长期权限边界无变化'); return }
  ...
  const save = () => {
    ...
    dispatch({ type: 'SAVE_AGENT_CONFIG', input: { agentId: agent.id, kind: 'permissions', value: draft }, summary: '收紧长期权限' }); commit('长期权限边界已保存到当前页面', false)
  }
  const restoreRevision = async (confirmationRef?: string) => {
    ...
    try {
      ...
      if (result.kind === 'saved' || result.kind === 'unchanged') { commitValue(parsed, selectedContent, result.kind === 'saved' ? '长期权限边界已恢复为新的配置版本' : '长期权限边界已是目标版本'); setHistoryOpen(false); return }
  ...
  const confirm = async () => {
    ...
    dispatch({ type: 'SAVE_AGENT_CONFIG', input: { agentId: agent.id, kind: 'permissions', value: draft }, summary: '确认扩大长期权限' }); commit('长期权限边界已在当前页面扩大', false)
  }
  const openHistory = async () => {
  ...
  const selectRevision = async (revision: ConfigRevisionDto) => { setBusy(true); try { setSelectedRevision(revision); setSelectedContent(await readConfigRevisionContent(revision.id)); setRestoreConfirmed(false) } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)) } finally { setBusy(false) } }
  const recoverRevision = async () => { if (!editor || !recoveryRef) return; setBusy(true); try { const result = await recoverConfigRevision({ requestId: `recover-permissions-${agent.id}`, assetId: editor.asset.id, recoveryRef }); if (result.kind === 'saved' || result.kind === 'unchanged') commit('长期权限边界配置版本已补记'); else setError(result.diagnostics.map((item) => item.message).join('；')) } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)) } finally { setBusy(false) } }
  ...
  return <><div className="grid gap-5 lg:grid-cols-2"><section className="panel p-5"><div className="flex items-center gap-2"><ShieldCheck className="text-success" aria-hidden="true" /><b>长期权限边界</b></div><p className="mt-3 text-sm leading-6 text-muted-foreground">这里设置 Agent 的长期能力上限和默认策略。实际工具调用仍由 Claude Code 在终端中按当前任务请求授权。</p><FieldRow label="文件写入">{canonical.files === '未授予' ? '默认不允许' : canonical.files}</FieldRow><FieldRow label="命令">{canonical.commands}</FieldRow><FieldRow label="网络">{canonical.network}</FieldRow><FieldRow label="委派">{canonical.delegation}</FieldRow></section><section className="panel p-5"><div className="label">调整长期边界</div><p className="mt-3 text-sm leading-6 text-muted-foreground">{desktopManaged ? '收紧后可直接保存；扩大长期能力上限需要高风险确认，但不会自动授权任何终端操作。' : '浏览器演示只更新当前页面，不会写入真实配置或授权终端操作。'}</p>{editing ? <><label className="mt-5 block text-sm font-medium">文件写入<select value={draft.files} onChange={(event) => setDraft((value) => ({ ...value, files: event.target.value }))} className="mt-2 h-10 w-full px-3"><option value="未授予">默认不允许</option><option>只读当前工作区</option><option>仅当前工作区</option><option>任意目录</option></select></label><div className="mt-4 flex gap-2"><Button variant="outline" disabled={busy} onClick={reset}>取消</Button><Button disabled={!dirty || busy} onClick={save}>{busy ? '保存中…' : desktopManaged ? '保存长期边界' : '保存到当前页面'}</Button></div></> : <Button className="mt-5" variant="outline" disabled={busy} onClick={beginEditing}><KeyRound size={16} aria-hidden="true" />{busy ? '加载中…' : '调整长期边界'}</Button>}</section></div><div className="mt-4 flex justify-end"><Button variant="ghost" size="sm" disabled={busy || editing} onClick={openHistory}><History size={14} aria-hidden="true" />版本历史</Button></div>{error && <div role="alert" className="mt-5 rounded-lg border border-danger/30 bg-danger/5 p-4 text-sm text-danger"><p>{error}</p>{recoveryRef && <Button className="mt-3" variant="outline" size="sm" disabled={busy} onClick={recoverRevision}>补记配置版本</Button>}</div>}{conflict && <div className="mt-5 panel p-4"><div className="grid gap-3 lg:grid-cols-3" aria-label="长期权限外部变化比较">{([{ label: '开始编辑时', side: conflict.base }, { label: '磁盘当前内容', side: conflict.current }, { label: '你的拟议内容', side: conflict.proposed }] as const).map(({ label, side }) => <section key={label} className="min-w-0 rounded-lg border border-border p-3"><b className="text-xs">{label}</b><pre className="mt-2 max-h-64 overflow-auto whitespace-pre-wrap break-words text-xs leading-5">{side.content}</pre></section>)}</div><div className="mt-3 flex justify-end"><Button variant="outline" size="sm" disabled={busy} onClick={reloadConflict}>加载磁盘当前内容</Button></div></div>}<AppDialog open={confirmOpen} onOpenChange={(open) => { setConfirmOpen(open); if (!open) { setConfirmName(''); setUnderstood(false) } }} title="确认扩大 Agent 长期权限" description={desktopManaged ? '本次确认只扩大长期能力上限，保存后立即失效；当前任务仍需在 Claude Code 终端授权。' : '仅在当前页面更新，不会写入真实配置或授权终端操作。'} footer={<><Button variant="outline" disabled={busy} onClick={() => setConfirmOpen(false)}>返回编辑</Button><Button variant="danger" disabled={!confirmed || busy || (desktopManaged && !challengeRef)} onClick={confirm}>{busy ? '保存中…' : '确认扩大长期边界'}</Button></>}><div className="rounded-lg border border-danger/30 bg-danger/5 p-4 text-sm leading-6"><b>影响范围</b><p className="mt-2 text-muted-foreground">Agent：{agent.name}<br />文件写入：{canonical.files} → {draft.files}<br />{desktopManaged ? '只修改长期配置，不表示当前会话或进程权限已变化。' : '仅更新当前页面，刷新后恢复初始状态。'}</p></div><label className="mt-5 block text-sm font-medium" htmlFor="permission-confirm-name">输入 Agent 名称“{agent.name}”确认<input id="permission-confirm-name" className="mt-2 h-10 w-full px-3" value={confirmName} onChange={(event) => setConfirmName(event.target.value)} autoComplete="off" /></label><label className="mt-4 flex items-start gap-3 text-sm leading-6"><input className="mt-1" type="checkbox" checked={understood} onChange={(event) => setUnderstood(event.target.checked)} /><span>我理解这是长期权限扩大，不是普通配置更新，也不会自动影响当前运行期。</span></label></AppDialog><AppDialog open={historyOpen} onOpenChange={(open) => { setHistoryOpen(open); if (!open) { setSelectedRevision(undefined); setRestoreConfirmed(false) } }} title="长期权限版本历史" description="历史版本不可变；恢复会生成新的配置版本。恢复到更宽边界仍需一次性确认，且不影响当前运行期。" size="xl" footer={<><Button variant="outline" onClick={() => setHistoryOpen(false)}>关闭</Button><Button disabled={!selectedRevision || selectedContent === canonicalContent || !restoreConfirmed || busy} onClick={() => void restoreRevision()}>恢复为新版本</Button></>}>
