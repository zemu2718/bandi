...
async function loadManagedAgentAssetEditor(
  ...
  const discovery = await discoverConfig({ requestId: `discover-${requestId}` })
...
function IdentityTab({ agent }: { agent: FullAgent }) {
  ...
  const saveLifecycle = async () => {
    ...
    try {
      ...
      const result = await saveAgentLifecycle({ agent, status: lifecycleTarget, requestId: lifecycleRequestId.current, dispatch })
      if (result.kind === 'baseline_changed') {
        setDraft({ ...canonical, status: lifecycleTarget }); setIdentityEditor(result.loaded); setIdentityConflict(result.result); setEditing(true); setLifecycleTarget(undefined); lifecycleRequestId.current = undefined; setSaveError('agent.yaml 已在生命周期确认期间发生变化。请基于磁盘当前内容重新核对。')
      } else if (result.kind === 'failed') {
        if (result.recoveryRef) { setIdentityEditor(await loadManagedAgentIdentity(agent.id)); setRecoveryRef(result.recoveryRef) }
        setSaveError(result.message)
      } else {
        setLifecycleTarget(undefined); lifecycleRequestId.current = undefined
      }
    } catch (error) { setSaveError(error instanceof Error ? error.message : String(error)) }
  ...
  return <><section className="panel overflow-hidden"><TabHeader title="身份与职责" description="加入 Team 不会自动获得权限。" editing={editing} onEdit={beginEdit} onCancel={cancel} onSave={save} canSave={!nameError} saveLabel={managedAvatar ? '保存' : '保存到当前页面'} />
    ...
    </div></section><AppDialog open={Boolean(lifecycleTarget)} onOpenChange={(open) => { if (!open) setLifecycleTarget(undefined) }} title={agent.status === 'archived' && lifecycleTarget === 'inactive' ? '移回当前 Agent' : lifecycleTarget === 'archived' ? '归档 Agent' : lifecycleTarget === 'inactive' ? '停用 Agent' : '重新启用 Agent'} description="此操作会把 Agent 状态保存到主配置文件。" footer={<><Button variant="outline" onClick={() => setLifecycleTarget(undefined)}>取消</Button><Button variant={lifecycleTarget === 'archived' ? 'danger' : 'default'} disabled={lifecycleSaving} onClick={saveLifecycle}>{lifecycleSaving ? '保存中…' : agent.status === 'archived' && lifecycleTarget === 'inactive' ? '移回当前 Agent' : '确认更新'}</Button></>}><div className="rounded-lg border border-border bg-muted/35 p-4 text-sm leading-6"><b>保留与影响</b><p className="mt-2 text-muted-foreground">Agent 配置、长期记忆和版本历史都会保留。停用或归档后，此 Agent 不能用于新的 AI 工具启动；已有引用不会自动删除。</p>{agent.status === 'archived' && lifecycleTarget === 'inactive' && <p className="mt-2 text-muted-foreground">移回后保持停用，需要重新启用才能使用。</p>}</div></AppDialog><AppDialog open={historyOpen} onOpenChange={(open) => { setHistoryOpen(open); if (!open) { setSelectedRevision(undefined); setRestoreConfirmed(false) } }} title="身份与职责版本历史" description="历史版本不可变；恢复仅写入完整的 Agent 主配置文件，并生成新的配置版本。头像文件不在历史中。" size="xl" footer={<><Button variant="outline" onClick={() => setHistoryOpen(false)}>关闭</Button><Button disabled={!selectedRevision || selectedContent === identityEditor?.canonicalContent || !restoreConfirmed || historyLoading} onClick={restoreIdentityRevision}>恢复为新版本</Button></>}>
...
function RulesTab({ agent, mode = 'rules' }: { agent: FullAgent; mode?: 'rules' | 'skills' | 'mcp' | 'sop' }) {
  ...
  const [selectedContent, setSelectedContent] = useState('')
  const [restoreConfirmed, setRestoreConfirmed] = useState(false)
  const sharedIds = new Set(state.sharedAssets.filter((item) => item.teamId === agent.teamId && item.kind === (mode === 'rules' ? 'rule' : mode === 'skills' ? 'skill' : mode) && item.parseStatus === 'parsed').map((item) => item.id))
  const candidates = state.assets.filter((item) => item.kind === assetKind && (state.runtime === 'web' || (item.teamId === agent.teamId && (sharedIds.has(item.id) || refs.includes(item.id)))))
  const missingRefs = refs.filter((id) => !candidates.some((item) => item.id === id))
  const proposedContent = serializeAgentConfig(agent, { kind: mode, value: refs }) ?? ''
  const emptyDescription = state.runtime === 'desktop'
    ? `当前没有可引用的共享${label}。请先前往“配置资产”新增或导入。`
