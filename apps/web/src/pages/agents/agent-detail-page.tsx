...
function IdentityTab({ agent }: { agent: FullAgent }) {
  ...
  const canonical: AgentIdentityConfig = useMemo(() => ({ schemaVersion: 1, id: agent.id, name: agent.name, status: agent.status, teamId: agent.teamId, avatarPath: agent.avatarPath, functionId: agent.functionId, mission: agent.mission, responsibilities: agent.responsibilities, deliverables: agent.deliverables, decisionBoundaries: agent.decisionBoundaries, escalationConditions: agent.escalationConditions, prohibitions: agent.prohibitions, completionDefinition: agent.completionDefinition }), [agent])
