...
export function AgentsPage() {
  ...
  const createActions = <div className="flex flex-wrap justify-center gap-2">{createAgentAction}<Button variant="outline" asChild><Link to="/agents/new?mode=import"><Upload size={16} aria-hidden="true" />导入 Agent</Link></Button></div>
  const firstUse = !teamAgents.length && !state.agentDiagnostics.length
  return <>
    <PageHeader title="Agent" description="新建或导入长期 Agent，安全管理自身配置与版本。" action={firstUse ? undefined : createActions} />
    {firstUse ? <div className="first-use-empty grid place-items-center"><EmptyState className="w-full max-w-lg border-0" title="还没有 Agent" action={<div className="flex flex-col items-center gap-1">{createAgentAction}<Button variant="ghost" asChild><Link to="/agents/new?mode=import"><Upload size={16} aria-hidden="true" />导入已有 Agent</Link></Button></div>} /></div> : <section className="panel overflow-hidden">
